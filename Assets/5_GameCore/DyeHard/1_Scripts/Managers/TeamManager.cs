...
namespace RedGaint.Games.DyeHard
{
    ...
    public class TeamManager : SingletonSimple<TeamManager>, IBugsBunny
    {
        ...
        private List<Action> teamAddedCallbacks = new List<Action>();

        public void RegisterTeamNotification(Action callback)
        {
            if (callback != null && !teamAddedCallbacks.Contains(callback))
            {
                teamAddedCallbacks.Add(callback);
            }
        }
        
        private Dictionary<GlobalEnums.GameTeam, int> internalAssignmentCounter = new();

        public GlobalEnums.GameTeam GetBalancedRandomTeam()
        {
            var enumValues = Enum.GetValues(typeof(GlobalEnums.GameTeam));
            int minCount = int.MaxValue;
            List<GlobalEnums.GameTeam> candidates = new();

            foreach (GlobalEnums.GameTeam team in enumValues)
            {
                if (team == GlobalEnums.GameTeam.None)
                    continue;

                int count = internalAssignmentCounter.TryGetValue(team, out var val) ? val : 0;
            ...
            {
                Debug.LogWarning("No eligible teams found in enum.");
                return GlobalEnums.GameTeam.None;
            }

            var selectedTeam = candidates[UnityEngine.Random.Range(0, candidates.Count)];

            if (!internalAssignmentCounter.ContainsKey(selectedTeam))
                internalAssignmentCounter[selectedTeam] = 0;

            internalAssignmentCounter[selectedTeam]++;
            return selectedTeam;
        }
        ...
        {
            internalAssignmentCounter.Clear();
        }
        private void NotifyTeamAdded()
        {
            foreach (var callback in teamAddedCallbacks)
            {
                callback?.Invoke();
            }
        }
